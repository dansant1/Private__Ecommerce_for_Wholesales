...
function SubirFoto (event, template, productoId) {
    ...
    if ('files' in archivo) {
        ...
        if (archivo.files.length == 0) {
          ...
          for (var i = 0; i < archivo.files.length; i++) {
            ...
            FotosProducto.insert(doc, function (err, fileObj) {
              if (err) {
...
function SubirBanner (event, template) {
    ...
    if ('files' in archivo) {
        ...
        if (archivo.files.length == 0) {
           alert('Selecciona una foto, vuelve a intentarlo', 'warning');
        } else if (archivo.files.length > 1) {
           alert('Selecciona solo una foto, vuelve a intentarlo', 'warning');
        } else {
          ...
          for (var i = 0; i < archivo.files.length; i++) {
           ...
           /* doc.metadata = {
            ...
            Banners.insert(doc, function (err, fileObj) {
              if (err) {
                alert('Hubo un problema', 'warning');
              } else {
                console.log('Listo!');
...
function SubirFotoVendedor (event, template, vendedorId) {
    ...
    if ('files' in archivo) {
        ...
        if (archivo.files.length == 0) {
          ...
          for (var i = 0; i < archivo.files.length; i++) {
            ...
            Logos.insert(doc, function (err, fileObj) {
              if (err) {
                alert('Hubo un problema', 'warning');
              } else {
                console.log('Listo!');
...
Template.ordenesAdmin.events({
	'click .p': function(event) {	
 	...
 	'click .ver': function () {
 		FlowRouter.go('/admin/ordenes/' + this._id);
 	}
});

Template.OrdenAdmin.onCreated(() => {
	let template = Template.instance();
...
Template.FormularioClientes.onCreated( () => {
	let template = Template.instance();

	template.autorun( () => {
		template.subscribe('vendedores');
	});
});

Template.FormularioClientes.helpers({
	vendedores() {
		return Vendedores.find();
	}
});

Template.FormularioClientes.events({
	'submit form'(e, t) {
		e.preventDefault();
		...
		console.log(datos.vendedor.id);

		if (datos.negocio !== "" && datos.email !== "" && datos.password) {
			Meteor.call('crearCliente', datos, function (err) {
				if (err) {
					console.log(err);
				} else {
					//t.find('[name="negocio"]').value = "";
...
Template.FormularioVendedor.events({
	'submit form': function (e, t) {
		...
		if (datos.nombre !== "" && datos.telefono !== "" && datos.direccion !== "" && datos.email !== "") {
			Meteor.call('agregarVendedor', datos, function (err, result) {
				if (err) {
					console.log(err);
				} else {
					t.find("[name='nombre']").value = ""
...
Template.ListaClientes.events({
  'click .borrar': function (e, t) {
    console.log('borrar');
    Meteor.call('eliminarCliente', this._id, function (err) {
      if (err) {
        console.log(err);
      }
    });
...
Template.ListaCategorias.events({
   'click .borrar': function (e, t) {
    console.log('borrar');
    Meteor.call('eliminarCategoria', this._id, function (err) {
      if (err) {
        console.log(err);
      }
    });
...
Template.ListaCondicionesDePago.events({
   'click .borrar': function (e, t) {
    console.log('borrar');
    Meteor.call('eliminarPago', this._id, function (err) {
      if (err) {
        console.log(err);
      }
    });
...
Template.ListaAsesores.onCreated( () => {
  let template = Template.instance();
...
Template.ListaAsesores.helpers({
  asesores: function () {
    return Vendedores.find();
  }
});

Template.ListaAsesores.events({
  'click .borrar': function (e, t) {
    console.log('borrar');
...
Template.ListaBanner.onCreated(() => {
  let template = Template.instance();
...
Template.ListaBanner.helpers({
  banners: function () {
    return Banners.find();
  }
});

Template.ListaBanner.events({
  'click .borrar': function (e, t) {
    console.log('borrar');
...
Template.FormularioProductos.onCreated( () => {
	let template = Template.instance();
...
Template.FormularioProductos.events({
	'submit form': function (e, t) {
		e.preventDefault();
		...
		if ( document.getElementById("n").checked == true) {
			//console.log($('input.novedades').checked);
			datos.novedad = true;
		} else {
			//console.log($('input.novedades').checked);
			datos.novedad = false
		...
		datos.categoriaId = $( "#categorias" ).val();

		if (datos.nombre !== "" && datos.descripcion !== "" && datos.precio !== "") {
			Meteor.call('crearProducto', datos, function (err, result) {
				if (err) {
					console.log(err);
				} else {
					SubirFoto(e, t, result.productoId);
					t.find('[name="nombre"]').value = "";
...
Template.FormularioBanner.events({
...
Template.FormularioCatergorias.events({
	'submit form': function (e, t) {
		e.preventDefault();

		let nombre = t.find("[name='categoria']").value;


		if (nombre !== "") {
			Meteor.call('crearCaterogia', nombre, function (err) {
				if (err) {
					console.log(err);
				} else {
					t.find("[name='categoria']").value = ""
				}
			});
		}
	}
...
Template.ListaCategorias.onCreated(function () {
	let self = this;
...
// Condiciones de pago
Template.FormularioCondicionesDePago.events({
	'submit form': function (e, t) {
		e.preventDefault();

		let nombre = t.find("[name='condicion']").value;


		if (nombre !== "") {
			Meteor.call('agregarCondicionDePago', nombre, function (err) {
				if (err) {
					console.log(err);
				} else {
					t.find("[name='condicion']").value = ""
				}
			});
		}
	}
...
Template.ListaCondicionesDePago.onCreated(function () {
	let self = this;
